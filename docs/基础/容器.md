...
- 在函数内使用`yield`关键字, 把普通函数变为生成器函数
    ...
    ???+ tip "Tip"
        ...
        生成器函数的执行流程和普通函数的执行流程不一样. 普通函数是顺序执行, 遇到`return`或者最后一行就返回. 但是生成器函数, 在每次调用`next()`的时候执行, 遇到`yield`语句中断, 再次执行时从上次返回的`yield`语句的下一句开始执行. 
        ...
        ???+ example "例子"
            ...
            可以看到, 在每一次调用`next()`的过程中, 遇到`yield`就中断, 再次执行时从上次返回的`yield`语句的下一句开始执行. 
...
而生成器不但可以作用于for循环, 还可以被`next()`函数不断调用并返回下一个值, 知道最后抛出`StopIteration`错误表示溢出. 
...
也就是说, 生成器都是`Iterator`对象, 但是列表, 字典, 字符串虽然是可迭代对象, 但不是迭代器.
...
???+ note "笔记"
    ...
    因为迭代器对象表示的是一个数据流, 我们无法提前知道序列的长度, 只能不断通过`next()`函数实现按需计算下一个数据. 
