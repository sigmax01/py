...
???+ danger "特别注意"
    ...
    - 所有被导入的模块都会被放在`sys.modules`中, 重复导入这些模块不会被执行, 包括`__init__.py`
...
???+ example "例子"
    ...
    ???+ question "疑问"
        ...
        为什么`__main__.py`的`__name__`被设置为`__main__`, 而不是`pkg.__main__`. 这和`__package__`为`pkg`有点自相矛盾.

        初步估计: `__main__.py`应该是直接被运行的, 所以`__name__`是`__main__`. `-m`的作用使得`__main__.py`中的`__package__`变为`pkg`而不是`None`[^8].
