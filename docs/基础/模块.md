...
???+ warning "注意"
    ...
    需要先安装`setuptools`模块: `pip install setuptools`.
...
???+ example "例子"
    ...
    === "`package_data`"
        ...
        这种情况下, 如果你运行`python setup.py sdist`, 默认情况下, 源分发包只会包含`mypackage`目录以及其中的模块, 其他的文件, 如`data/config.json`和`data/sample_data.csv`不会包含在源分发包中. 为了包含着两个文件, 使用了`package_data`指定这些文件需要包含到打包好的包里面. 
    ...
    === "`MANIFEST.in`"
        ...
        这种情况下, 如果你运行`python setup.py sdist`, 默认情况下, 源分发包只会包含`mypackage`目录以及其中的模块, 其他的文件, 如`README.md`和`data/dataset.csv`不会包含在源分发包中.
...
可以通过`python setup.py sdist`生成原分法包, 或者使用`python setup.py bdist`生成二进制分发包, 打包后的文件放在dist文件夹中, 扩展名分别为`.tar.gz`和`.whl`. 
