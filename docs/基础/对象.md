...
在Python中, 类是在运行时创建的, 不管是已经写死的类也好还是用`type()`动态创建的类也罢. 这里, 我们着重看动态创建的类, 因为它比较灵活, 可以在运行的时候根据用户的输入, 程序的状态确定一个类的结构. 这个和[反射](https://www.tutorialspoint.com/python/python_reflection.htm)密切相关. 
...
???+ tip "Tip"
    ...
    正常情况下, 写死的类如`class [class] ...`其实也是通过`type()`创建的, 区别就是一开始结构已经写死了, 但是后续其结构是否发生变化取决于是否使用反射. 
